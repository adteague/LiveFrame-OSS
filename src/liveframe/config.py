"""Configuration: settings, enums, and defaults for Liveframe."""
...
class LiveframeSettings(BaseSettings):
    """All configurable settings for Liveframe.

    Values are resolved in order: CLI flags > environment variables > defaults.
    Environment variables use the LIVEFRAME_ prefix (e.g., LIVEFRAME_GEMINI_MODEL).
    """

    model_config = {"env_prefix": "LIVEFRAME_", "env_file": ".env", "env_file_encoding": "utf-8"}
    ...
    def resolve_api_key(self) -> str:
        ...
        if not key:
            raise ValueError(
                "No Gemini API key found. Set GEMINI_API_KEY or LIVEFRAME_GEMINI_API_KEY "
