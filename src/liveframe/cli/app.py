"""Liveframe CLI — Typer application."""
...
def _build_settings(
    ...
    return LiveframeSettings(**overrides)
...
def analyze(
    ...
    manifest_dir = Path("liveframe_output") / input_video.stem
...
def extract(
    ...
    if output_dir is None:
        output_dir = Path("liveframe_output") / input_video.stem
...
def serve(
    ...
    """Start the Liveframe REST API server."""
    _setup_logging(verbose)
    import uvicorn

    console.print(f"Starting Liveframe API server on {host}:{port}...")
    uvicorn.run(
        "liveframe.api.app:create_app",
        factory=True,
        host=host,
        port=port,
        reload=reload,
