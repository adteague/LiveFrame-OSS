"""REST API endpoints for Liveframe."""
...
async def get_session_defaults(job_id: str) -> ClipRenderSettings:
    ...
    output_dir = Path(job.request.output_dir) if job.request.output_dir else Path("liveframe_output") / input_path.stem
    manifest_path = output_dir / "manifest.json"
...
async def render_clip_endpoint(
    ...
    from liveframe.core.renderer import render_clip
...
def _transcript_cache_path(job_id: str, clip_index: int) -> Path:
    ...
    return Path(".liveframe/transcripts") / f"{job_id}_{clip_index}.json"
