#!/usr/bin/env bash
#
# Liveframe - Guided Setup
# Run: bash setup.sh
#
# This script walks you through installing prerequisites and configuring
# Liveframe so it's ready to process your first video.
#
set -eo pipefail
...
if command -v liveframe &>/dev/null; then
    ok "CLI available: $(which liveframe)"
else
    fail "liveframe CLI not found after install. Make sure the venv is active."
    exit 1
fi
